...
POOL_TYPE     = "mean"   # "finaltoken", "mean", or "max"
BATCH_SIZE    = 2
MAX_LEN       = 2048
...
def beavertails_categories(hf_split):
    """Stable, sorted list of category names."""
    keys = list(hf_split[0]["category"].keys())
    keys.sort()
    return keys

def categories_to_onehot(cat_dicts, category_names):
    """List[Dict[str,bool]] -> (B, C) np.int8 one-hot matrix (multi-label)."""
    idx = {k: i for i, k in enumerate(category_names)}
...
def preprocess(batch, category_names):
...
def prepare_dataset(ds_split, category_names):
    ds = ds_split.map(
        lambda batch: preprocess(batch, category_names),
...
def save_all_layers_one_pass(loader, split_name, num_categories):
    ...
    try:
        ...
        for h in handles:
            h.remove()
        for mm in (*X_maps.values(), y_map, y_cat_map):
            mm.flush()
